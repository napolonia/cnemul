...
class Mininet( object ):
	...
	if not sta.func:
        ...
        ## RANGE!?!?!? should it come from sta.range?    
        #value = deviceRange(sta)
        #sta.range = value.range-15
    ...
    def ping( self, hosts=None, timeout=None, manualdestip=None, times=1 ):
	...
	if 'min_x' in kwargs:
	    mobility.MIN_X = kwargs['min_x']
	if 'min_y' in kwargs:
	    mobility.MIN_Y = kwargs['min_y']
        mobility.DRAW = True
        #if self.isVanet == False:
        #if self.ifaceConfigured == False:
        for node in self.wifiNodes:
            plot.instantiateGraph(mobility.MAX_X, mobility.MAX_Y, mobility.MIN_X, mobility.MIN_Y)
